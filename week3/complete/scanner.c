...
void skipBlank() {//bo qua khoang trang
  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_SPACE))
    readChar();
}

void skipComment() {//nhan biet cau comment (* *)
  int state = 0;
...
Token* readNumber(void) {
  ...
  if (count > 1 && charCodes[currentChar] == CHAR_RPAR && token->string[count-1] == '.'){
    jump(-1);
    count--;
    countPeriod--;
  }

  if (countPeriod == 0 || countPeriod == -1){
    token->string[count] = '\0';
  ...
  } else error(ERR_INVALIDNUMBER, token->lineNo, token->colNo);
  return token;
}
 ...
 int i = 0;
    ...
    while (i < MAX_IDENT_LEN && currentChar != EOF && charCodes[currentChar] != CHAR_DOUBLEQUOTE) {
        ...
        if(currentChar=='\n') {
          ...
          else if(currentChar == 'n') token->string[i++] = 10;
          else if(currentChar == '\\') token->string[i++] = 92;
...
Token* readConstChar(void) {
  ...
  if(currentChar != CHAR_SINGLEQUOTE) {
    token->string[i++] = currentChar;
    readChar();
  }
  else {
...
void printToken(Token *token) {
  ...
  case SB_SINGLEQUOTE: printf("SB_SINGLEQUOTE\n"); break;
